export default function TrendWidget({ title, value, statusText, statusType, chartType, data = [] }) {  const isUp = statusType === 'up';
      ...
      <div className="relative z-10 flex justify-between items-start mb-1">
        ...
        {statusText && (
          <div className="flex flex-col items-end">
            <span className={`text-sm font-bold ${isUp ? 'text-[#34D399]' : 'text-red-400'}`}>
              {statusText.split(' vs ')[0]}
            </span>
            <span className={`text-[11px] font-medium mt-0.5 ${isUp ? 'text-[#34D399]' : 'text-red-400'}`}>
        ...
        {chartType !== 'inventory' ? (
          <svg viewBox="0 0 100 40" preserveAspectRatio="none" className="w-full h-full">
            <defs>
              <linearGradient id={gradientId} x1="0" x2="0" y1="0" y2="1">
                <stop offset="0%" stopColor={currentStyle.stroke} stopOpacity="0.8" />
                <stop offset="100%" stopColor={currentStyle.stroke} stopOpacity="0" />
              </linearGradient>
            </defs>
            <path d={`${curvePath} L100,40 L0,40 Z`} fill={`url(#${gradientId})`} />
            <path d={curvePath} fill="none" stroke={currentStyle.stroke} strokeWidth="2.5" />
          </svg>
        ) : (
          <div className="w-full h-full flex items-end px-6 pb-8">
            <div className="w-full h-3 bg-[#0B1220] rounded-full overflow-hidden border border-[#7F92BB]/20 shadow-inner">
              <div className="h-full bg-[#10B981] rounded-full shadow-[0_0_12px_#10B981]" style={{ width: value }}></div>
            </div>
          </div>
        )}
      </div>
