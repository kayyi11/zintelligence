    ...
    <div className="bg-[#1F2937] rounded-xl border border-[#7F92BB]/40 shadow-2xl overflow-hidden">
      ...
      <div className="p-6 border-b border-[#7F92BB]/20 flex justify-between items-center bg-[#1F2937]">
        <div className="flex space-x-3">
          <button className="px-4 py-2 bg-[#0B1220] border border-[#7F92BB]/30 rounded-lg text-xs font-bold text-slate-300 hover:text-white transition-colors">
            + Add Row
          </button>
          <button className="px-4 py-2 bg-[#0B1220] border border-[#7F92BB]/30 rounded-lg text-xs font-bold text-slate-300 hover:text-white transition-colors">
            + Add Column
        ...
        <div className="flex space-x-3">
          <button className="px-6 py-2 bg-transparent text-[#2563EB] font-bold text-sm hover:bg-blue-500/5 rounded-lg transition-colors">
            Reset
          </button>
          <button className="px-6 py-2 bg-[#2563EB] text-white font-bold text-sm rounded-lg shadow-lg transition-all duration-300 ease-out hover:bg-[#3B82F6] active:scale-95">
            {" "}
            Save Changes{" "}
          </button>
        </div>
            ...
            {data.map((row, idx) => (
              ...
              >
                ...
                <td className="px-6 py-4">
                  <div
                    className={`mx-auto w-24 py-1.5 px-3 rounded-md text-center flex items-center justify-center space-x-2 border-2 ${
                      row.id === 1
                ...
                <td className="px-6 py-4 text-slate-300 text-center font-mono">
                  {(row.price * parseFloat(row.qty)).toFixed(2)}
                </td>
                ...
                <td className="px-6 py-4 text-right">
                  <span
                    className={`px-3 py-1 rounded-md font-bold text-[11px] border ${
                      row.conf >= 90
      ...
      <div className="p-6 bg-[#0B1220]/20 flex justify-between items-center">
        ...
        <div className="flex space-x-2">
          {[1, 2, 3, 4, 5].map((p) => (
            <button
              key={p}
              className={`w-8 h-8 rounded-md text-xs font-bold transition-all ${p === 1 ? "bg-[#2563EB] text-white" : "text-slate-500 hover:text-white"}`}
            >
              {p}
            </button>
          ))}
          <button className="w-8 h-8 rounded-md text-slate-500 hover:text-white">
            {" > "}
          </button>
        </div>
      </div>
